...
class HairpinComplex(ParentComplex):
    ...
    def __init__(self,  theta, strand1, loop, strand2,   T,concentration, sodium, magnesium , flurPosition,  dataset_name, docID, kinetic_model):
...
def main(real_rate, theta , beta , loop , gamma ,T, concentration, sodium, magnesium  , flurPosition , dataset_name, docID, kinetic_model):
    strand  = MyStrand(beta)
    strandComplement=MyStrand(gamma)
    loop = MyStrand(loop)
    helix_complex = HairpinComplex(theta, strand,  loop,strandComplement,  T, concentration, sodium, magnesium, flurPosition, dataset_name, docID, kinetic_model)
    bimolTransition  = False 
    return helix_complex.rate_constant(concentration , real_rate, bimolTransition, kinetic_model)
