...
class   ThreeWayBranchComplex(ParentComplex)  :
	...
	def __init__( self, beta, gamma,toehold_length, hasincumbentDangle, incumbentDangle,  dangleleft, dangleright, theta, substrate, attacker, incumbent,T, concentration, sodium, magnesium , dataset_name, docID , name  , substrateDangle, mismatchPosition, kinetic_model):
	...
	def dot_paren( self, state):
		...
		attacker = '.' * (L - j) + '(' * (j - i) + '.' * i
		if self.name =="Zhang" or self.name =="Machinek":
			substrate =  ('.' * i       + ')' * (j - i) + '.' * (k - j) +
					  '(' * (l - k) + '.' * (L - l))
		if self.name =="ReynaldoSequential":
...
def main(hasincumbentDangle,  incumbentDangle,real_rate, toehold_length,beta,gamma,  theta, T, concentration, sodium, magnesium   , dangle,  dataset_name, docID, name , incumbentMachinek, targetMachinek, invaderMachinek , mismatchMachinek, kinetic_model):
	...
	if name == "ReynaldoSequential" :
		...
		substrateDangle =""
		mismatchPosition =""
	threewaybranch_complex = ThreeWayBranchComplex(beta, gamma, toehold_length ,  hasincumbentDangle, incumbentDangle, dangleleft, dangleright,  theta,substrate,
		attacker, incumbent, T, concentration, sodium, magnesium , dataset_name, docID  , name , substrateDangle, mismatchPosition, kinetic_model)
	bimolTransition  = True
 
	return threewaybranch_complex.rate_constant(concentration, real_rate, True, kinetic_model)
